...
class SlideKitkat extends Visibility {
    ...
    public SlideKitkat(Context context, AttributeSet attributeSet) {
        ...
        setSlideEdge(obtainStyledAttributes.getInt(C0393R.styleable.lbSlide_lb_slideEdge, 80));
        long j = obtainStyledAttributes.getInt(C0393R.styleable.lbSlide_android_duration, -1);
        if (j >= 0) {
            setDuration(j);
        }
        long j2 = obtainStyledAttributes.getInt(C0393R.styleable.lbSlide_android_startDelay, -1);
        if (j2 > 0) {
            setStartDelay(j2);
        }
        int resourceId = obtainStyledAttributes.getResourceId(C0393R.styleable.lbSlide_android_interpolator, 0);
        if (resourceId > 0) {
    ...
    private Animator createAnimation(View view, Property<View, Float> property, float f, float f2, float f3, TimeInterpolator timeInterpolator, int i) {
        float[] fArr = (float[]) view.getTag(C0393R.C0396id.lb_slide_transition_value);
        if (fArr != null) {
            f = View.TRANSLATION_Y == property ? fArr[1] : fArr[0];
            view.setTag(C0393R.C0396id.lb_slide_transition_value, null);
        }
    ...
    public static class SlideAnimatorListener extends AnimatorListenerAdapter {
        ...
        public void onAnimationCancel(Animator animator) {
            this.mView.setTag(C0393R.C0396id.lb_slide_transition_value, new float[]{this.mView.getTranslationX(), this.mView.getTranslationY()});
