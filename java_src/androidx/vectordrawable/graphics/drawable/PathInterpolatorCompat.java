...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (Math.abs(this.f88mX[0]) <= 1.0E-5d && Math.abs(this.f89mY[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f88mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f89mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < min) {
        ...
        int i6 = min - 1;
        throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f88mX[0] + "," + this.f89mY[0] + " end:" + this.f88mX[i6] + "," + this.f89mY[i6]);
    }
