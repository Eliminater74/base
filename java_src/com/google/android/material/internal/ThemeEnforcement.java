...
public final class ThemeEnforcement {
    ...
    private static final int[] APPCOMPAT_CHECK_ATTRS = {C0974R.attr.colorPrimary};
    private static final int[] MATERIAL_CHECK_ATTRS = {C0974R.attr.colorPrimaryVariant};
    ...
    private static void checkCompatibleTheme(Context context, AttributeSet attributeSet, int i, int i2) {
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C0974R.styleable.ThemeEnforcement, i, i2);
        boolean z = obtainStyledAttributes.getBoolean(C0974R.styleable.ThemeEnforcement_enforceMaterialTheme, false);
        obtainStyledAttributes.recycle();
        if (z) {
            TypedValue typedValue = new TypedValue();
            if (!context.getTheme().resolveAttribute(C0974R.attr.isMaterialTheme, typedValue, true) || (typedValue.type == 18 && typedValue.data == 0)) {
    ...
    private static void checkTextAppearance(Context context, AttributeSet attributeSet, int[] iArr, int i, int i2, int... iArr2) {
        boolean z;
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C0974R.styleable.ThemeEnforcement, i, i2);
        if (!obtainStyledAttributes.getBoolean(C0974R.styleable.ThemeEnforcement_enforceTextAppearance, false)) {
            obtainStyledAttributes.recycle();
            return;
        }
        if (iArr2 == null || iArr2.length == 0) {
            z = obtainStyledAttributes.getResourceId(C0974R.styleable.ThemeEnforcement_android_textAppearance, -1) != -1;
        } else {
